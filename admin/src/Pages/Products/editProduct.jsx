...
const EditProduct = () => {
    ...
    useEffect(() => {
        ...
        fetchDataFromApi(`/api/product/${context?.isOpenFullScreenPanel?.id}`).then((res) => {
            ...
            setFormFields({
                ...
                brand: typeof res?.product?.brand === 'object' ? res?.product?.brand?._id : res?.product?.brand,
                brandName: typeof res?.product?.brand === 'object' ? res?.product?.brand?.name : res?.product?.brandName || "",
                ...
                isPopular: res?.product?.isPopular,
            ...
            const brandId = typeof res?.product?.brand === 'object' ? res?.product?.brand?._id : res?.product?.brand;
            // Only set the brand if it's a valid ID, otherwise set empty string
            setProductBrand(brandId || "");
            setProductThirdLavelCat(res?.product?.thirdsubCatId);
            setProductFeatured(res?.product?.isFeatured)
            setProductPopular(res?.product?.isPopular)
    ...
    // Enforce brand restrictions for brand owners
    useEffect(() => {
        if (context.userData?.role === 'BRAND_OWNER' && context.userData?.brandId && formFields.brand) {
            // If brand owner is editing a product that doesn't belong to their brand, redirect
            if (formFields.brand !== context.userData.brandId) {
                context.alertBox("error", "You can only edit products from your own brand.");
                history('/products');
                return;
            }
            
            // Ensure the brand field is locked to their brand
            if (context.brandData?.length > 0) {
                const userBrand = context.brandData.find(brand => brand._id === context.userData.brandId);
                if (userBrand && formFields.brandName !== userBrand.name) {
                    setFormFields(prev => ({
                        ...prev,
    ...
    }, [context.userData, context.brandData, formFields.brand, formFields.brandName]);
    ...
    const handleChangeProductBrand = (event) => {
        // Prevent brand owners from changing their brand
        if (context.userData?.role === 'BRAND_OWNER') {
            context.alertBox("error", "Brand owners cannot change their brand assignment.");
            return;
        }
    ...
    const handleSubmitg = (e) => {
        ...
        // Additional validation for brand owners
        if (context.userData?.role === 'BRAND_OWNER') {
            if (formFields.brand !== context.userData.brandId) {
                context.alertBox("error", "You can only edit products from your own brand.");
                return false;
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {previews?.length !== 0 && previews?.map((image, index) => (
                                <div className="relative group" key={index}>
                                    ...
                                    <div className="w-full aspect-square overflow-hidden border-2 border-gray-200 bg-white transition-shadow">
                                        <img 
                                            src={image} 
                                            alt={`Product ${index + 1}`}
