...
/**
 * Edit Hidden Gem Brand Component
 * Form to edit existing hidden gem brands
 */
const EditHiddenGemBrand = () => {
    ...
    const fetchBrandData = () => {
        ...
        fetchDataFromApi(`/api/hiddenGemBrands/${id}`).then((res) => {
            if (res.success && res.data) {
                const brand = res.data;
                setFormData({
                    brandName: brand.brandName || '',
                    brandId: brand.brandId?._id || '',
                    position: brand.position || 'small',
                    image: brand.image || '',
            ...
            setIsDataLoading(false);
            context.alertBox("error", "Failed to fetch brand data");
        });
    };
    ...
    const updateBrand = (e) => {
        ...
        editData(`/api/hiddenGemBrands/${id}`, formData).then((res) => {
            setIsLoading(false);
            if (res.success) {
                context.alertBox("success", "Hidden gem brand updated successfully");
                navigate('/hiddenGemBrands');
            } else {
                context.alertBox("error", res.message || "Failed to update hidden gem brand");
            }
        }).catch((error) => {
            setIsLoading(false);
            context.alertBox("error", "Failed to update hidden gem brand");
    ...
    return (
        <div className="right-content w-100">
            <div className="card shadow border-0 w-100 flex-row p-4">
                <h5 className="mb-0">Edit Hidden Gem Brand</h5>
            </div>

            <form className="form" onSubmit={updateBrand}>
                <div className="row">
...
export default EditHiddenGemBrand;
