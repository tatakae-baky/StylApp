...
/**
 * Edit SubCategory Section Component
 */
export const EditSubCategorySection = () => {
    ...
    useEffect(() => {
        ...
        // Fetch subcategory section data
        fetchDataFromApi(`/api/subCategorySections/get/${id}`).then((res) => {
            const data = res.subCategorySection;
            setFormFields(data);
            setProductCat(data.catId);
            setProductSubCat(data.subCatId);
            setProductThirdLevelCat(data.thirdsubCatId);
            ...
            context.alertBox("error", "Failed to fetch subcategory section data");
            setIsDataLoading(false);
        });
    }, [id]);
    ...
    const removeImg = (image, index) => {
        ...
        deleteImages(`/api/subCategorySections/deteleImage?img=${image}`).then((res) => {
    ...
    const handleSubmit = (e) => {
        ...
        if (!formFields.title.trim()) {
            context.alertBox("error", "Please enter a title");
            setIsLoading(false);
            return false;
        }
        ...
        if (!formFields.catId && !formFields.subCatId && !formFields.thirdsubCatId) {
            context.alertBox("error", "Please select at least one category");
            setIsLoading(false);
            return false;
        }

        editData(`/api/subCategorySections/update/${id}`, formFields).then((res) => {
            setTimeout(() => {
                setIsLoading(false);
                context.alertBox("success", "SubCategory section updated successfully");
                history("/subcategory-sections/list");
            }, 2500);
        }).catch((error) => {
            setIsLoading(false);
            context.alertBox("error", "Failed to update subcategory section");
        });
    };
    ...
    return (
        <section className='p-5 bg-gray-50'>
            <h2 className="text-xl font-semibold mb-4">Edit SubCategory Section Item</h2>
            <p className="text-gray-600 mb-6">Update navigation item for the subcategory section</p>
            ...
            <form className='form py-1 p-1 md:p-8 md:py-1 bg-white rounded-lg shadow' onSubmit={handleSubmit}>
                <div className='scroll max-h-[72vh] overflow-y-scroll pr-4 pt-4'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-3 gap-5'>
                        ...
                        <div className='col'>
                            ...
                            <input 
                                ...
                                placeholder="e.g., Westernwear, Footwear, Beauty"
                                required
                            />
                        </div>
                    ...
                    {/* Third Level Category, Sort Order and Status */}
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 mb-3 gap-5'>
                        <div className='col'>
                        ...
                        <div className='col'>
                            ...
                            <Select
                                ...
                                onChange={(e) => setFormFields(prev => ({ ...prev, isActive: e.target.value }))}
                            >
                    ...
                    <h3 className='text-[18px] font-[500] mb-0 text-black'>Category Image *</h3>
                    <br />
                    <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
                        {previews?.length !== 0 && previews?.map((image, index) => (
                            <div className="uploadBoxWrapper mr-3 relative" key={index}>
                                ...
                                <div className='uploadBox p-0 rounded-md overflow-hidden border border-dashed border-[rgba(0,0,0,0.3)] h-[150px] w-[100%] bg-gray-100 cursor-pointer hover:bg-gray-200 flex items-center justify-center flex-col relative'>
                                    <img src={image} className='w-100 h-full object-cover' alt="Category" />
                                </div>
                            </div>
                        ))}
                        ...
                        <UploadBox 
                            multiple={false} 
                            name="images" 
                            url="/api/subCategorySections/uploadImages" 
