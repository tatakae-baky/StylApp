...
/**
 * Edit Hero Banner Component
 * Form for editing existing hero banners for the 70-30 layout
 */
export const EditHeroBanner = () => {
    const { id } = useParams();
    ...
    const fetchBannerData = async () => {
        try {
            ...
            const res = await fetchDataFromApi(`/api/heroBanners/${id}`);
            if (res?.data) {
    ...
    const handleChangePosition = (event) => {
        ...
        setFormFields(prev => ({ ...prev, position: event.target.value }));
    };
    ...
    const handleSubmit = (e) => {
        ...
        if (!formFields.position) {
            context.alertBox("error", "Please select banner position");
            setIsLoading(false);
            return false;
        }

        editData(`/api/heroBanners/${id}`, formFields).then((res) => {
            setTimeout(() => {
                setIsLoading(false);
                context.alertBox("success", "Hero banner updated successfully");
                history("/hero-banners/list");
            }, 2500);
        }).catch((error) => {
            setIsLoading(false);
            context.alertBox("error", "Failed to update hero banner");
        });
    };
    ...
    return (
        <section className='p-5 bg-gray-50'>
            <h2 className="text-xl font-semibold mb-4">Edit Hero Banner</h2>
            
            <form className='form py-1 p-1 md:p-8 md:py-1 bg-white rounded-lg shadow' onSubmit={handleSubmit}>
                <div className='scroll max-h-[72vh] overflow-y-scroll pr-4 pt-4'>
                    
                    <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mb-3 gap-5'>
                        
                        {/* Title Input (Optional) */}
                        <div className='col'>
                            <h3 className='text-[14px] font-[500] mb-1 text-black'>Banner Title (Optional)</h3>
                            <input 
                                type="text" 
                        ...
                        {/* Position Selection */}
                        <div className='col'>
                            <h3 className='text-[14px] font-[500] mb-1 text-black'>Banner Position *</h3>
                            <Select
                            ...
                            >
                                <MenuItem value="main">Main Hero (70% width)</MenuItem>
                                <MenuItem value="side">Side Hero (30% width)</MenuItem>
                            </Select>
                        </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-7 gap-4">
                        {previews?.length !== 0 && previews?.map((image, index) => (
                            <div className="uploadBoxWrapper mr-3 relative" key={index}>
                                ...
                                <div className='uploadBox p-0 rounded-md overflow-hidden border border-dashed border-[rgba(0,0,0,0.3)] h-[150px] w-[100%] bg-gray-100 cursor-pointer hover:bg-gray-200 flex items-center justify-center flex-col relative'>
                                    <img src={image} className='w-100 h-full object-cover' alt="Hero Banner" />
                                </div>
                            </div>
                        ))}
                        ...
                        <UploadBox 
                            multiple={false} 
                            name="images" 
                            url="/api/heroBanners/uploadImages" 
