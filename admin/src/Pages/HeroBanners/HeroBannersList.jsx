...
/**
 * Hero Banners List Component
 * Displays and manages hero banners for the 70-30 layout
 */
export const HeroBannersList = () => {
    ...
    /**
     * Fetch hero banners data
     */
    const getData = () => {
        setIsLoading(true);
        fetchDataFromApi("/api/heroBanners").then((res) => {
    ...
    const deleteBanner = (id) => {
        if (context?.userData?.role === "ADMIN") {
            if (window.confirm("Are you sure you want to delete this hero banner?")) {
                deleteData(`/api/heroBanners/${id}`).then((res) => {
                    context.alertBox("success", "Hero banner deleted successfully");
                    getData();
                }).catch((error) => {
                    context.alertBox("error", "Failed to delete hero banner");
                });
            }
        } else {
                                        ...
                                        <TableRow hover role="checkbox" tabIndex={-1} key={banner._id}>
                                            ...
                                            <TableCell>
                                                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                                    ...
                                                    {banner.position === 'main' ? 'Main (70%)' : 'Side (30%)'}
                                                </span>
                                            </TableCell>
                                            ...
                                            <TableCell>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <Link to={`/hero-banners/edit/${banner._id}`}>
