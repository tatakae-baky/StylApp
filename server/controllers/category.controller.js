...
export async function removeImageFromCloudinary(request, response) {
    try {
        ...
        // Admin-only validation (enforced by route middleware, but double-check)
        if (user.role !== 'ADMIN') {
            return response.status(403).json({
                error: true,
                success: false,
                message: "Only administrators can delete category images"
            });
        }

        // Verify the image belongs to a category in our system
        ...
        if (!category) {
            return response.status(404).json({
                error: true,
                success: false,
                message: "Image not found in any category"
            });
        }
